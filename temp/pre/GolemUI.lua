...
--Core.MainFrame = GolemUI_Main_Frame
...
function Antispam.ContainBadWords(message)
	for word in string.gmatch(message, "[1234567890abcdefghijklmnopqrstuvwxyzrабвгдеёжзийклмнопрстуфхцчшщъьыэюяABCDEFGHIJKLMNOPQRSTUVWXYZRАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЬЫЭЮЯ]+") do
		if inTable(Filters.BadWords, string.lower(word)) then return true, word end
	end
	return false, nil
end
...
function eventHandlers.CHAT_MSG_SYSTEM(msg, sender, ...)
	Core.MainFrame:UnregisterEvent("CHAT_MSG_SYSTEM");
	label, access = strsplit(":", msg);
	--print(msg)
	--print(label)
	--print(access)
	Core.access = access * 1024
end

function eventHandlers.PLAYER_FLAGS_CHANGED(...)
	--print ("PLAYER_FLAGS_CHANGED fired")
	--SendChatMessage("testtest", "SAY", nil, nil);	
	--angle = math.random(1, 360)
	--print("Rotation angle="..angle)
	--FlipCameraYaw(angle)
	--RunScript("JumpOrAscendStart()")
	--SendChatMessage("/script JumpOrAscendStart()", "SAY", nil, nil);	
	--SetCVar("cameraYawMoveSpeed","8");
	--MoveViewRightStart();
	--SpinCamRunning = true;
end
Core.MainFrame:RegisterEvent("PLAYER_FLAGS_CHANGED");
Core.MainFrame:RegisterEvent("CHAT_MSG_SYSTEM");
...
function eventHandlers.ADDON_LOADED(...)
	...
	Core.SettingsCheckSAY:SetPoint("TOPLEFT", 20, -30)
	...
	Core.SettingsCheckWHISPER:SetPoint("TOPLEFT", 20, -50)
	...
	Core.SettingsCheckPARTY:SetPoint("TOPLEFT", 20, -70)
	...
	Core.SettingsCheckRAID:SetPoint("TOPLEFT", 20, -90)
	...
	Core.SettingsCheckGUILD:SetPoint("TOPLEFT", 20, -110)
	...
	Core.SettingsCheckCHANNEL:SetPoint("TOPLEFT", 20, -130)
	...
	Core.SettingsAbuseTextBtn:SetPoint("TOP", 0, -160)
	...
	Core.SettingsAdvertisersBtn:SetPoint("TOP", 0, -190)
	...
	Core.SettingsBadWordsBtn:SetPoint("TOP", 0, -220)
	...
	Core.SettingsSellersBtn:SetPoint("TOP", 0, -250)
	...
	SVcount = 1
	--table.insert(savedVariablesFrames, SVFrame)
	table.insert(savedVariablesFrames, SV)
	...
	function Core.AddonEnable()
		if Core.access ~= 2 then 
			Core.BigBro()
			return 
		end
		Settings.running = true
		Core.btnOn:LockHighlight()
		Core.btnOn:Disable()
		Core.btnOff:UnlockHighlight()
		Core.btnOff:Enable()
		-- Показываем фрейм
		--UIFrameFadeIn(GolemUI_Main_Frame, 1.34);		
		if Settings.scanSAY == true then 
			Core.MainFrame:RegisterEvent("CHAT_MSG_SAY");
		else 
			Core.MainFrame:UnregisterEvent("CHAT_MSG_SAY"); 
		end;
		
		if Settings.scanWHISPER == true then 
			Core.MainFrame:RegisterEvent("CHAT_MSG_WHISPER");
		else 
			Core.MainFrame:UnregisterEvent("CHAT_MSG_WHISPER"); 
		end;
		
		if Settings.scanPARTY == true then 
			Core.MainFrame:RegisterEvent("CHAT_MSG_PARTY");
		else 
			Core.MainFrame:UnregisterEvent("CHAT_MSG_PARTY"); 
		end;
		
		if Settings.scanRAID == true then 
			Core.MainFrame:RegisterEvent("CHAT_MSG_RAID");
		else 
			Core.MainFrame:UnregisterEvent("CHAT_MSG_RAID"); 
		end;
		...
		if Settings.scanGUILD == true then 
			Core.MainFrame:RegisterEvent("CHAT_MSG_GUILD");
		else 
			Core.MainFrame:UnregisterEvent("CHAT_MSG_GUILD"); 
		end;
		
		if Settings.scanCHANNEL == true then 
			Core.MainFrame:RegisterEvent("CHAT_MSG_CHANNEL");
		else 
			Core.MainFrame:UnregisterEvent("CHAT_MSG_CHANNEL"); 
		end;		
	...
	function Core.BigBro()
		Core.AddonDisable()
		Core.btnOn:Hide()
		Core.btnOff:Hide()
		Core.btnLogs:Hide()
		Core.btnSettings:Hide()
		Core.MainFrame:Hide()
	end
